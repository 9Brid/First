...
public class Q06 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        BufferedInputStream inputStream = new BufferedInputStream(new FileInputStream("src/han01/q06.txt"));
    ...
    static private boolean judge(int x, int y, int oldx, int oldy, int z) {
        if(x<0 || x>=m || y<0 || y>=n) {
            return false;
        }
        if((g[oldx][oldy] >> z & 1) != 0) {
            return false;
        }
        if(inq[x][y]) {
            return false;
